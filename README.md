...
## 🧩 Что удобно показать учителю

- Переключение вкладок на странице iPhone.
- Выбор количества гигабайт и изменение цены.
- Переключение языка и светлой / тёмной темы.
- Переходы по страницам социальной сети.
- Поиск друзей: список меняется сразу при вводе текста.
- Лайки: число увеличивается без перезагрузки страницы.
- Пустые состояния: профиль без постов и поиск без результатов.
- `routeRules` в `nuxt.config.ts`: `prerender`, `swr`, `ssr: false`, redirect.
